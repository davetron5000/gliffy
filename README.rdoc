...
Then, in your <tt>sessions_controller.rb</tt>:
    ...
    def create
      ...
      if logged_in?
        ...
        session[:gliffy] = Gliffy::Handle.new($gliffy_root,cred)
    end
...
The functional tests perform actual workflows against a Gliffy server and, as such, are more fragile.  If something is wrong in your environment (or with Gliffy), you could leave the test account in a weird state.
